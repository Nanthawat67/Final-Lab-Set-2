...
async function logActivity({ userId, username, eventType, entityId, summary, meta }) {
  ...
  }).catch(() => console.warn('[task] activity-service unreachable — skipping event log'));
}
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    // Log ทั้ง DB + Log Service + Activity
    await Promise.all([
      logToDB({ level: 'INFO', event: 'TASK_CREATED', userId: req.user.sub, message: `Task created: "${title}"`, meta: { task_id: task.id, title } }),
      logEvent({ level: 'INFO', event: 'TASK_CREATED', userId: req.user.sub, method: 'POST', path: '/api/tasks', statusCode: 201, message: `Task created: "${title}"`, meta: { task_id: task.id, title } }),
      logActivity({ userId: req.user.sub, username: req.user.username, eventType: 'TASK_CREATED', entityId: task.id, summary: `Task "${title}" created` })
    ]);
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    await Promise.all([
      logToDB({ level: 'INFO', event: 'TASK_UPDATED', userId: req.user.sub, message: `Task updated: "${task.title}"`, meta: { task_id: task.id } }),
      logEvent({ level: 'INFO', event: 'TASK_UPDATED', userId: req.user.sub, method: 'PUT', path: `/api/tasks/${id}`, statusCode: 200, message: `Task updated: "${task.title}"`, meta: { task_id: task.id } }),
      logActivity({ userId: req.user.sub, username: req.user.username, eventType: 'TASK_UPDATED', entityId: task.id, summary: `Task "${task.title}" updated` })
    ]);
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    await Promise.all([
      logToDB({ level: 'INFO', event: 'TASK_DELETED', userId: req.user.sub, message: `Task ${id} deleted`, meta: { task_id: id } }),
      logEvent({ level: 'INFO', event: 'TASK_DELETED', userId: req.user.sub, method: 'DELETE', path: `/api/tasks/${id}`, statusCode: 200, message: `Task ${id} deleted`, meta: { task_id: id } }),
      logActivity({ userId: req.user.sub, username: req.user.username, eventType: 'TASK_DELETED', entityId: id, summary: `Task ${id} deleted` })
    ]);
